...
figure;
plot (t, right); grid
title('RightStep05 - Right')

acceX = data.signals(1, 3).values;
idAcceX = iddata(acceX(1:end), u(1:end), 0.01);
valAcceX = iddata(acceX(n/2+1:end), u(n/2+1:end), 0.01);
figure;
plot (t, acceX); grid
title('RightStep05 - AcceX')
